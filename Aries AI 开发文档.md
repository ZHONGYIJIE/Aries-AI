...
- **开发文档** → [最新状态](#1-最新状态（2026-02-16）) | [代码入口](#2-关键代码入口与数据流索引) | [路线图](#3-近期路线图（2026-02-11-更新）)
...
- [版本升级指南](#版本升级指南)
- [快速查问题索引](#快速查问题索引)
- [1. 最新状态（2026-02-16）](#1-最新状态（2026-02-16）)
- [2. 关键代码入口与数据流索引](#2-关键代码入口与数据流索引)
- [3. 近期路线图（2026-02-11 更新）](#3-近期路线图（2026-02-11-更新）)
...
|--------|-------------|-----|--------|---------|-----------|---------|--------|------|
| **1.3.0** (当前) | 13 | 8.13.2 | 2.0.21 | 30 | 36 | 2026-02-16 | 虚拟屏功能完善、输入源管理优化、自动化全链路 | 现网 |
...
| 现象 | 根因 | 记录位置 | 状态 |
|------|------|--------|------|
| 小窗关闭后没有回到主界面 | ACK 机制超时 / 任务栈异常 | [1. 最新状态](#1-最新状态（2026-02-11）) | ✅ 已修复 |
...
| API 429 请求过于频繁 | 无重试 + 指数退避 | [1. 最新状态](#1-最新状态（2026-02-11）) | ✅ 已内置重试 |
| 虚拟屏关闭不停止 / 状态不刷新 / 分辨率漂移 | 事件未联动 + 坐标未对齐 | [1. 最新状态](#1-最新状态（2026-02-16）) | ✅ 已修复 |
| 虚拟屏输入默认剪贴板内容而非模型输出 | 输入源管理缺失 | [5.12](#512-虚拟屏输入默认剪贴板内容而非模型输出（待修复）) | ⏳ 待修复 |
| 自动化进度显示为"1/12"不对 | 无预估步数支持 | [1. 最新状态](#1-最新状态（2026-02-16）) | ✅ 已改百分比 |
...
**【问题记录重组】** 按模块分类
- 目标：6.1 UI 与交互 / 6.2 Gradle / 6.3 数据同步 / 6.4 语音 / 6.5 功能配置
- 进度：保留所有 10 个历史问题的详细定位修复过程，仅调整分类逻辑和导航索引
- 计划完成：2026-02-18

**【更新记录增强】** 时间轴 + 模块索引 双视图
- 进度：2026-01-11 条目已标记 【流式解析】【Markdown渲染】 等模块标签
- 需续：2026-01-09 及以后的条目补充标签，完成模块变更速查表
- 计划完成：2026-02-18
...
- 拖动吸边/停靠（类微信语音通话）
- 完成态光效与进度文案优化
- 小窗 vs 全屏返回 UX 优化（Android 14+ 后台启动限制适配）
