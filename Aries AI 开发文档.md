...
- **文档目的**：让后来者（包括其他 AI）能快速理解 Aries AI 当前实现到哪里、有哪些关键决策未定、以及遇到的问题是怎么定位/修复的。
- **阅读顺序建议**：先看「最新状态」→「代码入口」→「关键决策」→「路线图」→「开发构想」→「问题记录」→「更新记录」→「知识库」。
- **自动化对齐路线（Aries 方法论）**：见「4. 开发构想」与「3. 近期路线图」

## 目录

### 快速导航

- **版本管理** → [版本快照](#版本快照与兼容性矩阵) | [升级指南](#版本升级指南) | [快速查问题](#快速查问题索引)
- **开发文档** → [最新状态](#1-最新状态（2026-02-25）) | [代码入口](#2-关键代码入口与数据流索引) | [路线图](#3-近期路线图（2026-02-25-更新）)
- **问题与更新** → [问题记录](#5-问题记录（定位-→-修复）) | [更新记录](#6-更新记录（changelog）) | [知识库](#7-知识库（reference）)
- **其他** → [构想](#4-开发构想（aries-风格手机助手）) | [历史](#8-历史任务进度（旧）) | [LFS 提醒](#9-大文件lfs-与协作提醒)

### 完整目录

- [版本快照与兼容性矩阵](#版本快照与兼容性矩阵)
- [版本升级指南](#版本升级指南)
- [快速查问题索引](#快速查问题索引)
- [1. 最新状态（2026-02-25）](#1-最新状态（2026-02-25）)
- [2. 关键代码入口与数据流索引](#2-关键代码入口与数据流索引)
- [3. 近期路线图（2026-02-25 更新）](#3-近期路线图（2026-02-25-更新）)
- [4. 开发构想（Aries 风格手机助手）](#4-开发构想（aries-风格手机助手）)
- [5. 问题记录（定位 → 修复）](#5-问题记录（定位-→-修复）)
- [6. 更新记录（Changelog）](#6-更新记录（changelog）)
- [7. 知识库（Reference）](#7-知识库（reference）)
- [8. 历史任务进度（旧）](#8-历史任务进度（旧）)
- [9. 大文件/LFS 与协作提醒](#9-大文件lfs-与协作提醒)
...
| 版本号 | versionCode | AGP | Kotlin | Min SDK | Target SDK | 发布日期 | 主要功能 | 状态 |
|--------|-------------|-----|--------|---------|-----------|---------|--------|------|
| **1.3.2** (当前) | 15 | 8.13.2 | 2.0.21 | 30 | 36 | 2026-02-25 | Shizuku 交互优化、OpenAI 兼容 API、网站移动端优化 | 现网 |
| 1.3.0 | 13 | 8.13.2 | 2.0.21 | 30 | 36 | 2026-02-16 | 虚拟屏功能完善、输入源管理优化、自动化全链路 | 稳定 |
| 1.1.x | 10-11 | 8.12.x | 2.0.x | 30 | 35 | 2025-12-31 | 主对话页 UI、Drawer 手势、自动化闭环、权限体系 | 可用 |
| 1.0.x | 1-9 | 8.0.x | 1.9.x | 30 | 33 | 2025-12-01 | 初始版本、基础对话、自动化原型 | 弃用 |

### 依赖版本范围

| 组件 | 1.3.2 | 说明 |
|------|-------|------|
| JDK | 17 | 强制使用，避免 8/11/21 兼容性问题 |
| Android SDK | 36 (API Level) | 对标 Android 16，targetSdk 必须 ≥ 34 |
| Android NDK | 25.1.8937393 (可选) | 用于语音识别等 native 模块 |
| Gradle Wrapper | 8.13.x | 见 gradle/wrapper/gradle-wrapper.properties |
...
| 功能 | 变更 | 影响 | 迁移建议 |
|------|------|------|--------|
| **OpenAI 兼容 API** | 新增第三方 OpenAI 兼容 API 支持 | 可接入更多模型服务商 | 在设置中配置自定义 API 端点和密钥 |
| **Shizuku 交互** | 优化权限请求和服务连接流程 | 提升虚拟屏稳定性 | 重新授权 Shizuku 权限以获得最佳体验 |
| **推荐任务** | 日期改为"明天" | 避免过期日期混淆 | 无需迁移，自动生效 |
| **网站移动端** | 响应式布局优化 | 改善手机浏览体验 | 无需操作 |

#### 新增功能

- **多模型支持**：可配置 DeepSeek、Moonshot、通义千问等 OpenAI 兼容 API
- **Shizuku 稳定性提升**：虚拟屏与 Shizuku 协同更流畅
- **移动端文档**：官网和文档中心在手机上浏览更友好

#### 测试清单

- [ ] OpenAI 兼容 API：配置第三方端点，测试对话和自动化功能
- [ ] Shizuku 权限：重新授权后测试虚拟屏创建和操作
- [ ] 推荐任务：检查日期显示是否为"明天"
- [ ] 移动端浏览：在手机上访问官网和文档中心
...
| 功能 | 变更 | 影响 | 迁移建议 |
|------|------|------|--------|
| **流式解析** | 新增 `AriesStreamParser`，吞掉 `【回答开始/结束】` 标记 | 旧版输出协议与新 UI 不兼容 | 模型 system prompt 需更新为 Aries 格式；见第 1 节 |
| **Markdown 渲染** | 历史/最终消息统一用 `MarkdownRenderer`（Markwon） | 代码块/表格样式可能变化 | UI 测试覆盖所有消息类型（历史/流式结束） |
| **小窗消息同步** | 新增待同步队列 + ACK 机制 | 低端机可能同步延迟 | 建议使用 `FloatingChatService.messageSyncListener` 监听队列状态 |
| **主界面重试** | 重试绑定对应用户问题，而非最后一条 | 旧逻辑可能导致重试失败 | 确保每条 AI 气泡含 `linkedUserMessage` 信息 |

#### 测试清单

- [ ] 流式消息：思考/回答段落分离显示，无标记泄露
- [ ] 历史消息：Markwon 渲染与复制功能正常
- [ ] 小窗同步：发消息后立刻切换到主界面，消息应出现
- [ ] 重试功能：点击 AI 气泡重试，应重发对应用户问题

#### 已知问题

- **输入框焦点争抢**（已修复）：若仍有失焦现象，检查是否有自定义 ScrollView 调用 `fullScroll(FOCUS_DOWN)`
...
| 现象 | 根因 | 记录位置 | 状态 |
|------|------|--------|------|
| 小窗关闭后没有回到主界面 | ACK 机制超时 / 任务栈异常 | [1. 最新状态](#1-最新状态（2026-02-25）) | ✅ 已修复 |
| 输入框点击后光标闪动，无法输入 | ScrollView 抢焦点 | [5.10](#510-主界面输入框点击后偶现失焦（已修复）) | ✅ 已修复 |
| 右滑打开 Drawer 无响应 | 边缘检测逻辑缺失 | [5.9](#59-主界面右滑打开-drawer-无响应（已修复）) | ✅ 已修复 |
| 消息在小窗发出，主界面看不到（反之亦然） | 前缀解析不一致 + 监听器清理过早 | [5.8](#58-小窗主界面消息不同步或消息丢失显示（已修复）) | ✅ 已修复 |
| 语音识别"模型解压失败" | 缓存损坏或存储空间不足 | [5.5](#55-vosk-语音模型解压失败缓存损坏（已处理，详见知识库）) | ✅ 已处理 |
| API 429 请求过于频繁 | 无重试 + 指数退避 | [1. 最新状态](#1-最新状态（2026-02-25）) | ✅ 已内置重试 |
| 虚拟屏关闭不停止 / 状态不刷新 / 分辨率漂移 | 事件未联动 + 坐标未对齐 | [1. 最新状态](#1-最新状态（2026-02-25）) | ✅ 已修复 |
| 虚拟屏输入默认剪贴板内容而非模型输出 | 输入源管理缺失 | [5.12](#512-虚拟屏输入默认剪贴板内容而非模型输出（待修复）) | ⏳ 待修复 |
| 自动化进度显示为"1/12"不对 | 无预估步数支持 | [1. 最新状态](#1-最新状态（2026-02-25）) | ✅ 已改百分比 |
| Gradle 无法同步，ktlint 报错 | 插件版本/仓库问题 | [5.1](#51-gradle-sync-与-ktlint-插件解析问题（已修复）) | ✅ 已移除 ktlint |
| VSCode Kotlin 提示 403 | GitHub API 限流 | [5.4](#54-vscode-kotlin-language-server-403-github-api-rate-limit-exceeded（已解决）) | ✅ 已解决 |

---

<a id="1-最新状态（2026-02-25）"></a>

## 1. 最新状态（2026-02-25）

### 版本 1.3.2 发布 - Shizuku 交互优化与第三方 API 支持（2026-02-25）

- **Shizuku 交互模式优化（已完成）**：
  - 优化 Shizuku 权限请求流程，提升用户体验
  - 改进 Shizuku 服务连接稳定性
  - 增强虚拟屏幕与 Shizuku 的协同工作机制

- **第三方 OpenAI 兼容 API 支持（已完成）**：
  - 新增对第三方 OpenAI 兼容 API 的支持
  - 用户可配置自定义 API 端点
  - 支持更多大模型服务商接入（如 DeepSeek、Moonshot 等）
  - 统一的 API 调用接口，降低切换成本
...
- **自动化任务日期修正（已完成）**：
  - 修复推荐任务中日期显示问题
  - 将示例任务日期统一改为"明天"，避免过期日期导致的混淆

- **涉及文件**：
  - `app/build.gradle.kts` - 版本号更新至 1.3.2 (versionCode 15)
  - `app/src/main/java/com/ai/phoneagent/ShizukuBridge.kt` - Shizuku 交互优化
  - `app/src/main/java/com/ai/phoneagent/net/AutoGlmClient.kt` - OpenAI 兼容 API 支持
  - `app/src/main/java/com/ai/phoneagent/MainActivity.kt` - 主页面输入栏优化
  - `app/src/main/java/com/ai/phoneagent/AutomationActivityNew.kt` - 推荐任务日期修正
  - `Aries-site/` - 网站移动端优化
  - `README.md` - 文档更新

### 输入栏语音/键盘切换稳定性修复 + 关于页样式回滚（2026-02-19）

- **输入模式切换闪动/回退修复（已完成）**：
  - 现象：从语音切到键盘时出现闪动，随后又被回写到语音态。
  - 处理：在 `onModeChange` 中先统一停止语音录制/识别与动画，再更新 `InputState`，避免异步状态覆盖。
  - 结果：语音 ↔ 键盘切换更稳定，不再“切过去又弹回”。
- **“按住说话”位置与过渡优化（已完成）**：
  - 调整输入栏中部对齐与模式切换动画，保证语音文案在按钮区域稳定居中。
- **Compose 实验动画告警处理（已完成）**：
  - 在输入栏相关 Compose 组件补充 `@OptIn(ExperimentalAnimationApi::class)`，消除实验 API 使用告警。
- **关于页 UI 回滚（已完成）**：
  - `activity_about.xml` 已恢复至原始样式版本，撤回本轮视觉微调稿。
- **涉及文件**：
  - `app/src/main/java/com/ai/phoneagent/MainActivity.kt`
  - `app/src/main/java/com/ai/phoneagent/ui/inputbar/KimiInputBar.kt`
  - `app/src/main/res/layout/activity_about.xml`

### 虚拟屏输入问题识别与整合（2026-02-16）

- **问题现象**：虚拟屏中执行 `Type` 动作时，输入框默认接收剪贴板最新内容，而非 AI 模型输出的目标文本（如 12306 订座应输入的站点名字）。
- **影响范围**：自动化任务中所有涉及文本输入的场景；功能已实现但输入源管理流程需要审视。
- **当前状态**：基础虚拟屏与虚拟屏交互框架完成，需要修正输入文本源的管理逻辑。
- **后续计划**：
  - 检查 `Type` 动作实现中文本参数的传递与验证；
  - 确认输入框焦点隔离与剪贴板交互的边界；
  - 补充调试日志与输入验证机制。
- **详见**：[5.12 虚拟屏输入默认剪贴板内容而非模型输出（待修复）](#512-虚拟屏输入默认剪贴板内容而非模型输出（待修复）)

### 后台虚拟屏全链路优化（2026-02-16）

- **虚拟屏关闭按钮联动停止**：预览悬浮窗右上角“✕”会广播停止请求，自动停止当前任务并清理虚拟屏资源。
- **分辨率动态对齐**：每步动作执行前刷新 `ShizukuVirtualDisplayEngine.getLatestContentSize()`，坐标换算不再受分辨率变化影响。
- **截图尺寸同步**：虚拟屏截图宽高来自虚拟屏内容尺寸，避免固定值导致的坐标误差。
- **状态显示实时刷新**：自动化控制台增加周期刷新，保证“已启动/DisplayId”与后台实际一致。
- **关停路径一致**：任务结束/手动停止/悬浮窗关闭均执行同一清理流程（预览窗 + 虚拟屏）。
- **参考应用对齐点**：与 autoglm_KY 一致使用最新 content size 进行预览比例与坐标换算。
...
## 1.1 近期进度与关键变更（2026-01-13）

### 关于界面（About）优化
- 顶部栏采用 LinearLayout+Space+权重，彻底解决“关于”标题与返回按钮不显示问题，适配所有主流机型（含7寸大屏）。
- AppBarLayout 高度由固定 56dp 改为 wrap_content + minHeight，兼容状态栏沉浸式与刘海屏。
- 返回按钮图标填充色由白色改为蓝色（#6BA8FF），并在代码中用 ImageViewCompat 兼容性着色，保证所有主题下可见。
- “联系我们”邮箱文本与点击复制逻辑均已统一为 zhangyongqi@njit.edu.cn。
- 相关文件：
  - `app/src/main/res/layout/activity_about.xml`
...
### 自动化控制台与无障碍适配
- 自动化界面顶部标题已更名为“Aries AI自动化控制台”，风格与主界面一致。
- 无障碍服务未开启时，任务输入框自动提示“请打开无障碍服务”，并禁用输入，提升易用性。
- 推荐语句滚动展示、语音输入按钮优化、输入区布局精简，提升交互体验。
- 相关文件：
  - `app/src/main/res/layout/activity_automation.xml`
  - `app/src/main/java/com/ai/phoneagent/AutomationActivityNew.kt`

### 代码结构与体验提升
- 统一推荐语句系统，协程定时切换，支持一键填入。
- 语音输入、进度百分比、悬浮窗等体验持续优化。
- 兼容 Android 11-16，重点适配 Android 14+ 后台启动限制。

---

## 1.2 项目结构与核心模块梳理（2026-01-13）

- `MainActivity.kt`：主对话界面，负责消息流、AI气泡渲染、历史记录、侧边栏等。
- `AutomationActivityNew.kt`：自动化控制台，集成无障碍、推荐语句、语音输入、进度反馈。
- `AboutActivity.kt`：关于界面，展示项目信息、开发者、联系方式。
- `FloatingChatService.kt`：小窗/悬浮窗服务，支持全局对话与自动化。
- `helper/`：流式解析、Markdown渲染、内容过滤等工具类。
- `updates/`：版本更新、历史弹窗、下载管理。
- `res/layout/`：所有主要界面布局 XML，已对齐 Material3 风格。
- `res/drawable/`：图标、圆角背景、按钮等资源，自动生成缺失项。
- `res/values/`：主题色、字符串、尺寸等全局资源。
...
1. **自动化闭环与稳定性**
   - 完善自动化流程的异常兜底与日志，提升复杂任务下的可追溯性。
   - 增强无障碍服务状态监听，支持动态授权/失效时的 UI 提示。
2. **多端适配与测试矩阵**
   - 持续在 Android 11/13/14/16 多机型回归，重点关注悬浮窗、后台启动、输入法兼容。
   - 增加自动化 UI 测试脚本，覆盖主流程。
3. **AI 能力扩展**
   - 引入本地/云端多模型切换，支持更丰富的推理场景。
   - 优化流式输出与 Markdown 渲染，提升长文本与代码块体验。
4. **用户体验细节**
   - 关于页增加版本号、隐私政策跳转等信息。
   - 自动化控制台支持任务模板/历史复用。
5. **文档与协作**
   - 每次大改动后同步更新开发文档，补充典型问题与解决方案。
   - 规范 PR 流程，推动代码评审与知识沉淀。

---

> 如需详细代码索引、历史变更、问题定位等，请查阅本文件后续章节与相关子文档。

<a id="2-关键代码入口与数据流索引"></a>
...
<a id="3-近期路线图（2026-02-11-更新）"></a>

## 3. 近期路线图（2026-02-25 更新）

### 3.1 已完成（2026-02-25）

- ✅ **版本 1.3.2 发布** — OpenAI 兼容 API、Shizuku 优化、网站移动端适配
- ✅ **第三方 API 支持** — 可接入 DeepSeek、Moonshot 等多种模型服务
- ✅ **Shizuku 交互优化** — 权限流程和服务连接稳定性提升
- ✅ **网站移动端优化** — 响应式布局改进，文档中心适配
- ✅ **主页面 UI 优化** — 输入栏对齐修复
- ✅ **推荐任务优化** — 日期显示改为"明天"

### 3.2 已完成（2026-02-11）

- ✅ **版本管理文档** — 版本快照、升级指南、快速查问题索引、模块变更速查表
- ✅ **编译验证** — :app:compileDebugKotlin 通过
- ✅ **版本号** — App 1.3.0 (versionCode 13, targetSdk 36, AGP 8.13.2, Kotlin 2.0.21)
...
**【虚拟屏输入源管理修复】**
- 目标：修复虚拟屏 Type 动作默认使用剪贴板而非模型输出的问题
- 进度：问题已识别，需要梳理输入文本源管理逻辑
- 计划完成：2026-03-05

**【本地编译与真机回归】**
- 编译：✅ 通过
- 回归测试：待 overlay/进度显示与重试功能日志验证（Android 14+ 设备优先）
...
**【自动化性能提速】**
- 本地优先 Launch（应用直启缓存，从 5-10s 降到 ~1s）
- 减少不必要的采集等待
- 本地模板动作补充（抖音点赞/评论等常见场景）

**【语音模块升级】**（可选）
- 从 Vosk → Sherpa-ncnn 的完整迁移与性能对标
- 或保持当前 Sherpa-ncnn 并补充模型优化

### 3.4 长期维护项

- 依赖版本更新跟进（AGP / Kotlin / Gradle Wrapper）
- 兼容 Android 11-16 设备测试矩阵维护（重点 Android 14+ 限制适配）
- 知识库文档补充（Reference 见第 7 节）


<a id="4-开发构想（aries-风格手机助手）"></a>
...
> 按模块分类，保留历史问题详细定位与解决方案；快速查问题见 [快速查问题索引](#快速查问题索引)。

### 5.1 Gradle Sync 与 ktlint 插件解析问题（已修复）
...
### 5.2 Drawer 沉浸式适配：状态栏文字/图标变白 & 灰遮罩残留（已规避）
...
### 5.3 VSCode 状态栏显示 “Gradle: Build Error”（无需处理）
...
### 5.4 VSCode Kotlin Language Server 403：GitHub API rate limit exceeded（已解决）
...
### 5.5 Vosk 语音模型解压失败/缓存损坏（已处理，详见知识库）
...
### 5.11 虚拟屏关闭按钮未停止任务 / 状态不刷新 / 分辨率漂移（已修复）

- **现象**：点击虚拟屏预览右上角“✕”后，虚拟屏仍在运行且任务未停止；控制台显示“未启动”；分辨率变化时坐标偏移。
- **影响**：自动化继续在后台执行且状态不可见，点击命中率降低。
- **根因**：关闭按钮未联动停止任务；UI 状态仅靠一次性日志更新；坐标换算仍使用主屏分辨率。
- **修复/规避**：
  - 预览关闭按钮广播停止请求，自动化与虚拟屏统一清理；
  - 运行期间周期刷新虚拟屏状态；
  - 每步刷新虚拟屏 content size 作为坐标换算基准；
  - 截图尺寸同步为虚拟屏内容尺寸。
- **涉及文件**：
  - `AutomationActivityNew.kt`
  - `VirtualScreenPreviewOverlay.kt`
  - `VirtualDisplayController.kt`
  - `UiAutomationAgent.kt`
  - `ScreenshotManager.kt`

### 5.12 虚拟屏输入默认剪贴板内容而非模型输出（待修复）

- **现象**：在虚拟屏中执行输入操作（`Type` 动作）时，输入框默认接收的是剪贴板中最新复制的内容，而不是 AI 模型输出的目标文本（如 12306 订座中应输入的高铁站点名字）。
- **影响**：
  - 自动化任务中涉及文本输入的场景失败或产生错误结果；
  - 用户无法有效控制虚拟屏上的文本输入，降低自动化可靠性。
- **根因**：
  - 虚拟屏输入的实现缺少明确的"输入源管理"，可能混淆了剪贴板粘贴与引擎提供的文本输入责任边界；
  - `Type` 动作执行时的文本参数传递/验证流程需要梳理。
- **分析思路**：
  1. 检查虚拟屏 `Type` 动作的实现：确认文本内容是否正确从 AI 动作参数传入；
  2. 检查输入目标（InputMethod/文本框焦点）的获取与验证逻辑；
  3. 排查是否有全局剪贴板监听或粘贴快捷键触发；
  4. 验证虚拟屏与主屏之间的输入框焦点隔离（是否会被主屏输入状态相互影响）。
- **后续计划**：
  - 增强 `Type` 动作的调试信息（打印目标文本/焦点状态/输入结果）；
  - 补充自动化日志，记录每步输入的预期值与实际值；
  - 考虑在虚拟屏中禁用全局粘贴快捷键或添加输入验证机制。
- **涉及文件**（预期）：
  - `ActionExecutor.kt` 或 `UiAutomationAgent.kt`（Type 动作执行）
  - `VirtualDisplayController.kt` 或 `ShizukuBridge.kt`（虚拟屏输入注入）
  - `PhoneAgentAccessibilityService.kt`（输入框焦点检测）

<a id="6-更新记录（changelog）"></a>
...
| 模块 | 最新版本 | 最新日期 | 改进摘要 |
|------|---------|--------|--------|
| **API 支持** | 1.3.2 | 2026-02-25 | 新增第三方 OpenAI 兼容 API 支持，可接入 DeepSeek、Moonshot 等 |
| **Shizuku 交互** | 1.3.2 | 2026-02-25 | 优化权限请求流程和服务连接稳定性 |
| **网站优化** | 1.3.2 | 2026-02-25 | 移动端响应式布局优化，文档中心适配 |
| **虚拟屏功能** | 1.3.0 | 2026-02-16 | 虚拟屏幕创建与管理、实时预览、后台运行、输入源管理 |
| **流式解析** | 1.3.0 | 2026-01-11 | AriesStreamParser 增强、Aries 标记对齐、吞掉标记 |
| **Markdown 渲染** | 1.3.0 | 2026-01-11 | Markwon 统一、历史消息渲染修复、复制功能优化 |
| **小窗实现** | 1.3.0 | 2026-01-09 | 回退稳定版、ACK 机制、防闪优化（120ms延迟） |
| **消息同步** | 1.3.0 | 2026-01-02 | 双向队列同步、前缀兼容、监听器生命周期优化 |
| **UI 交互** | 1.3.0 | 2026-02-19 | 输入栏语音/键盘切换稳定性修复，语音文案对齐优化，关于页样式回滚 |
| **自动化执行** | 1.3.0 | 2026-01-05 | 应用直启（1s启动）、敏感检测优化、百分比进度 |
| **对话管理** | 1.3.0 | 2026-01-08 | 上下文完整性、批量刷新、消息气泡操作区修复 |
| **权限体系** | 1.3.0 | 2026-01-01 | 首次启动权限引导、后台权限检查 |
| **工程构建** | 1.3.0 | 2026-01-01 | 移除 ktlint、修复 Gradle 同步 |
| **虚拟屏交互** | 1.3.0 | 2026-02-16 | 关闭按钮联动、分辨率对齐、状态刷新、输入源管理（待修复） |

### 更新记录 · 2026-02-25

- **【版本发布】1.3.2 版本发布**
  - versionCode 更新至 15
  - versionName 更新至 1.3.2
  - 涉及文件：`app/build.gradle.kts`

- **【API 支持】新增第三方 OpenAI 兼容 API**
  - 支持配置自定义 API 端点和密钥
  - 兼容 DeepSeek、Moonshot、通义千问等 OpenAI 格式 API
  - 统一的多模型接入接口
  - 涉及文件：`app/src/main/java/com/ai/phoneagent/net/AutoGlmClient.kt`

- **【Shizuku】交互模式优化**
  - 优化 Shizuku 权限请求流程
  - 改进服务连接稳定性
  - 增强虚拟屏与 Shizuku 协同机制
  - 涉及文件：`app/src/main/java/com/ai/phoneagent/ShizukuBridge.kt`

- **【网站】移动端浏览优化**
  - 优化官网响应式布局
  - 改进文档中心移动端显示
  - 提升手机端用户体验
  - 涉及文件：`Aries-site/index.html`, `Aries-site/docs-center.html`, `Aries-site/assets/app.js`

- **【UI】主页面输入栏对齐优化**
  - 修复输入栏在不同屏幕尺寸下的对齐问题
  - 优化输入框与按钮的视觉平衡
  - 涉及文件：`app/src/main/java/com/ai/phoneagent/MainActivity.kt`

- **【自动化】推荐任务日期修正**
  - 将示例任务日期统一改为"明天"
  - 避免过期日期导致的用户混淆
  - 涉及文件：`app/src/main/java/com/ai/phoneagent/AutomationActivityNew.kt`
...
- **【输入栏】语音/键盘切换防闪回修复**
  - 在 `onModeChange` 中先停止录音识别与相关动画，再切换 `InputState`，避免状态竞态导致的模式回退。
  - 涉及文件：`app/src/main/java/com/ai/phoneagent/MainActivity.kt`

- **【输入栏】“按住说话”对齐与过渡优化**
  - 调整输入栏中部布局与过渡动画，语音模式文案居中显示更稳定。
  - 涉及文件：`app/src/main/java/com/ai/phoneagent/ui/inputbar/KimiInputBar.kt`

- **【Compose】实验动画 API 告警处理**
  - 为动画相关 Composable 添加 `@OptIn(ExperimentalAnimationApi::class)`，清理编译告警。
  - 涉及文件：`app/src/main/java/com/ai/phoneagent/ui/inputbar/KimiInputBar.kt`

- **【关于页】界面恢复原样**
  - `activity_about.xml` 已回滚至原样式，撤回本轮视觉细节改动。
  - 涉及文件：`app/src/main/res/layout/activity_about.xml`

### 更新记录 · 2026-02-16

- **【虚拟屏】关闭按钮联动停止**：预览悬浮窗关闭将广播停止请求，自动停止任务并清理虚拟屏资源。
- **【虚拟屏】分辨率动态对齐**：每步刷新虚拟屏内容尺寸用于坐标换算，减少分辨率变化导致的点击偏移。
- **【虚拟屏】截图尺寸同步**：截图宽高来自虚拟屏内容尺寸，避免固定值导致误差。
- **【虚拟屏】状态实时刷新**：自动化控制台增加周期刷新，确保显示状态与后台一致。
- **【虚拟屏】输入问题识别**：发现虚拟屏 `Type` 动作默认接收剪贴板内容而非模型输出的问题，已整合到问题记录；涉及输入源管理流程梳理，后续修复。

- **涉及文件**：
  - `app/src/main/java/com/ai/phoneagent/AutomationActivityNew.kt`
  - `app/src/main/java/com/ai/phoneagent/VirtualScreenPreviewOverlay.kt`
  - `app/src/main/java/com/ai/phoneagent/VirtualDisplayController.kt`
  - `app/src/main/java/com/ai/phoneagent/UiAutomationAgent.kt`
  - `app/src/main/java/com/ai/phoneagent/core/cache/ScreenshotManager.kt`

### 更新记录 · 2026-01-11

**【流式解析】对话输出协议：Aries标记格式对齐**
  - 主对话系统提示更新：要求模型严格输出 `【思考开始/结束】` 与 `【回答开始/结束】`。
  - 涉及文件：`MainActivity.kt`

- **【流式解析】流式解析：吞掉回答开始/结束标记，避免 UI 泄露**
  - `AriesStreamParser` 增强：识别回答开始/结束标记并以控制信号驱动 UI 阶段切换。
  - 涉及文件：`helper/AriesStreamParser.kt`

- **【Markdown渲染】Markdown 渲染：历史/最终态统一为 Markwon**
...
- **【重试逻辑】主界面重试：对齐 DeepSeek regenerate（重发对应用户问题）**
...
- **【流式解析】结束态稳定性：flush 解析器残留 + 清理尾巴标记片段**
...
- **【UI/交互优化】自动化界面推荐语句滚动展示**
  - 在任务输入框下方添加半透明推荐语句区域，每4秒自动切换展示。
  - 用户可点击推荐语句直接填入输入框，提升输入效率。
  - 涉及文件：`AutomationActivityNew.kt`

- **【UI/交互优化】语音输入图标优化**
  - 语音输入按钮移至任务输入框右侧居中位置，更显眼易点击。
  - 涉及文件：`AutomationActivityNew.kt`

- **【代码结构】推荐语句系统**
  - 推荐语句列表集中管理，滚动逻辑与UI解耦，使用协程实现定时切换。
  - 涉及文件：`AutomationActivityNew.kt`
...
<a id="9-大文件lfs-与协作提醒"></a>
确认/接管机制**（敏感 Tap 确认、暂停/继续/接管）：pending
