...
object ShizukuVirtualDisplayEngine {
    ...
    fun start(args: Args = Args()): Result<Int> {
        Log.i(TAG, "ShizukuVirtualDisplayEngine.start: width=${args.width}, height=${args.height}, dpi=${args.dpi}")
    ...
    /**
     * 将系统焦点显示恢复到默认主屏（displayId = 0）。
     * 用于虚拟屏操作完成后，避免物理按键（如返回键）被路由到虚拟屏。
     */
    fun restoreFocusToDefaultDisplay(): Result<Unit> {
        return runCatching {
            ...
            Log.w(TAG, "restoreFocusToDefaultDisplay failed", t)
        }
    }
    ...
    private fun createVirtualDisplay(args: Args, surface: Surface): Pair<Int, Any> {
        ...
        val allCreateMethods = displayManager.javaClass.methods.filter { it.name == "createVirtualDisplay" }
        ...
        val createMethod = allCreateMethods.firstOrNull { m ->
            ...
            when (p.size) {
                4 -> {
                    // 标准签名: VirtualDisplayConfig, IVirtualDisplayCallback, IMediaProjection, String
                    p[0].name.contains("VirtualDisplayConfig") &&
                    (p[1].name.contains("IVirtualDisplayCallback") || p[1].name.contains("IInterface")) &&
                    (p[2].name.contains("IMediaProjection") || p[2] == java.lang.Void.TYPE || p[2] == null) &&
            ...
            allCreateMethods.firstOrNull() ?: throw NoSuchMethodException("No createVirtualDisplay method found")
        }

        Log.i(TAG, "Using createVirtualDisplay: ${createMethod.name}: ${createMethod.parameterTypes.map { it.name }.joinToString()}")
        ...
        val id = try {
            when (createMethod.parameterTypes.size) {
                4 -> createMethod.invoke(displayManager, config, callback, null, packageName) as Int
                3 -> createMethod.invoke(displayManager, config, callback, packageName) as Int
                else -> throw IllegalStateException("Unsupported parameter count: ${createMethod.parameterTypes.size}")
            }
        } catch (t: Throwable) {
            ...
            if (cause is SecurityException && trustedFlag != null && (flags and trustedFlag) != 0) {
                ...
                when (createMethod.parameterTypes.size) {
                    4 -> createMethod.invoke(displayManager, configNoTrust, callback, null, packageName) as Int
                    3 -> createMethod.invoke(displayManager, configNoTrust, callback, packageName) as Int
                    else -> throw IllegalStateException("Unsupported parameter count: ${createMethod.parameterTypes.size}")
                }
            } else {
                throw t
    ...
    private fun applyImePolicyBestEffort(displayId: Int) {
        runCatching {
            ...
            runCatching {
                if (!ShizukuBridge.pingBinder() || !ShizukuBridge.hasPermission()) return@runCatching
